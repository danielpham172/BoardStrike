...
public class HalberdierPiece extends Piece {

	public HalberdierPiece(Board board, int row, int col, int team) {
		super(board, row, col, team);
	}

	@Override
	public PieceType getType() {
		return PieceType.HALBERDIER;
	}
	...
	public ArrayList<MoveData> getPossibleMoves(boolean ignoreCheck) {
		...
		for (int i = 0; i < rowChange.length; i++) {
			moves.addAll(possibleLinearMoves(copyBoard, rowChange[i] * getForwardDirection(), colChange[i], 3, ignoreCheck));
		}
